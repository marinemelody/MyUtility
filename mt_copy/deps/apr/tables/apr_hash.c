...
#define APR_HASH_KEYDATA_MAXLEN     (32)

struct apr_hash_entry_t {
    apr_hash_entry_t *next;
    unsigned int      hash;
    //const void       *key;
    apr_byte_t        kdat[APR_HASH_KEYDATA_MAXLEN];
    apr_ssize_t       klen;
    const void       *val;
};
                                     ...
                                     const void *val)
{
    ...
    if (klen == APR_HASH_KEY_STRING) {
        klen = MIN(APR_HASH_KEYDATA_MAXLEN, strlen((char *)key));
    } else {
        klen = MIN(APR_HASH_KEYDATA_MAXLEN, klen);
    }
...
APR_DECLARE(void) apr_hash_clear(apr_hash_t *ht)
{
    ...
    for (hi = apr_hash_first(NULL, ht); hi; hi = apr_hash_next(hi)) {
        ...
        apr_hash_set(ht, hi->this->kdat, hi->this->klen, NULL);
    }
}
                             ...
                             void *rec, const apr_hash_t *ht)
{
    ...
    if ((hi = apr_hash_next(&hix))) {
        ...
        do {
            ...
            rv = (*comp)(rec, hi->this->kdat, hi->this->klen, hi->this->val);
